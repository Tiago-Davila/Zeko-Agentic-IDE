...
export function RuntimeCanvas({ approvals = [] }: { readonly approvals?: readonly ApprovalDto[] }) {
  ...
  return <section aria-label="Runtime Canvas"><p>Vista observacional: los nodos representan estado y no ejecutan workflows.</p><ProviderStatusPanel providers={[{ provider: 'Docker', state: 'unknown', detail: 'Esperando estado local' }, { provider: 'Ollama', state: 'unknown', detail: 'Esperando estado local' }]} /><div role="list" aria-label="Ejecuciones">{executions.map((execution) => <div key={execution.id} role="listitem"><ExecutionCard execution={execution} /><ExecutionControls execution={execution} onUpdated={replace} /><ExecutionResultPanel executionId={execution.id} /><TracePanel resourceId={execution.id} /></div>)}</div>{approvals.map((approval) => <ApprovalPrompt key={approval.id} approval={approval} />)}</section>;
}
