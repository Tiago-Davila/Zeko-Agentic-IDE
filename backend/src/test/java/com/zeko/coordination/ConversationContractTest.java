...
class ConversationContractTest {
  ...
  static void database(DynamicPropertyRegistry registry) {
    registry.add("zeko.datasource.path",
                 () -> localDataDir.resolve("conversation-api.db").toString());
  }
  @Test
  void rejectsUnknownConversationsThroughTheLocalApi() throws Exception {
    mockMvc
        .perform(get("/api/conversations/00000000-0000-0000-0000-000000000001")
                     .cookie(session()))
  ...
  private Cookie session() throws Exception {
    return mockMvc.perform(post("/api/session/bootstrap"))
        .andReturn()
        .getResponse()
        .getCookie(LocalSessionFilter.COOKIE_NAME);
  }
}
